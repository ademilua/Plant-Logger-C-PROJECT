	...
	for (int i = 0; i < Package->numberOfChannels; i++)
	{
		...
		for (int j = 0; j < NumberOfPoints; j++)
		{
			...
			else
			{
				...
				if (strcmp(PointName, "Input turbidity") == 0 ||
					strcmp(PointName, "Output turbidity") == 0)
				{
					sprintf_s(FileBuf, "%.4fNTU\r\n", dValue);
				}
				if (strcmp(PointName, "Pressure") == 0 ||
					strcmp(PointName, "Output air pressure") == 0)
				{
					sprintf_s(FileBuf, "%.4fatm\r\n", dValue);
				}
				//printf("%f\n", dValue);
				this->Print(logfile, FileBuf);
